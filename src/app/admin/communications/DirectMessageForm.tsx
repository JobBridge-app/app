...
export function DirectMessageForm() {
    ...
    async function handleSubmit(formData: FormData) {
        ...
        const emailOrId = formData.get("target") as string;
        const title = formData.get("title") as string;
        const body = formData.get("body") as string;

        try {
            const res = await sendDirectMessage(emailOrId, title, body);
            if (res.success) {
                setMessage({ type: 'success', text: res.message });
                (document.getElementById("dm-form") as HTMLFormElement).reset();
            } else {
    ...
    return (
        <form id="dm-form" action={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Recipient (Email or ID)</label>
                <input
                    name="target"
                    required
            ...
            <div>
                ...
                <textarea
                    ...
                    required
                    rows={4}
                    placeholder="We noticed unusual activity..."
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-white focus:outline-none focus:border-emerald-500 transition-colors resize-none"
                />
            </div>
