...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'profiles_mobile_nav_preference_check'
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_mobile_nav_preference_check
      CHECK (mobile_nav_preference IN ('top', 'bottom'));
  END IF;
END $$;
