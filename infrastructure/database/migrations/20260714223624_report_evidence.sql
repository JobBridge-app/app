-- Preserve the exact moderation context at report time. The product only
-- exposes this service-role-only evidence to the separate admin application.
...
BEGIN
  ...
  v_evidence := jsonb_strip_nulls(jsonb_build_object(
    ...
    'message', CASE WHEN v_target_type = 'message' THEN jsonb_build_object(
      'id', p_message_id,
    ...
    'reopen_request', CASE WHEN v_target_type = 'reopen_request' THEN jsonb_build_object(
